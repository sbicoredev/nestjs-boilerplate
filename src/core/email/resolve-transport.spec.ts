...
describe("resolveTransport", () => {
  it("returns the configured SMTP URL for provider=smtp", () => {
    const emailConfigs = baseEmailConfig({
      provider: "smtp",
      smtpUrl: "smtp://example:1025",
    });
    expect(resolveTransport(emailConfigs)).toBe("smtp://example:1025");
  });

  it("builds a SendGrid SMTP-relay URL for provider=sendgrid", () => {
    const emailConfiguration = baseEmailConfig({
      provider: "sendgrid",
      sendgridApiKey: "SG.abc123",
    });
    expect(resolveTransport(emailConfiguration)).toBe(
  ...
  it("throws for provider=sendgrid with no API key configured", () => {
    ...
    expect(() => resolveTransport(emailConfiguration)).toThrow(
      /SENDGRID_API_KEY/
    );
  });

  it("throws a clear not-implemented error for provider=ses", () => {
    const emailConfiguration = baseEmailConfig({ provider: "ses" });
    expect(() => resolveTransport(emailConfiguration)).toThrow(
      /not implemented/i
    );
  });
