...
export function resolveTransport(
  emailConfigs: Configurations["email"]
): string {
  switch (emailConfigs.provider) {
    case "smtp":
      return emailConfigs.smtpUrl;
    case "sendgrid":
      // SendGrid via its SMTP relay — no extra SDK dependency needed,
      // this is still "just" an SMTP transport under the hood.
      if (!emailConfigs.sendgridApiKey) {
        throw new Error(
          "EMAIL_PROVIDER=sendgrid requires SENDGRID_API_KEY to be set."
        );
      }
      return `smtps://apikey:${encodeURIComponent(emailConfigs.sendgridApiKey)}@smtp.sendgrid.net:465`;
    case "ses":
    ...
    default:
      throw new Error(`Unknown EMAIL_PROVIDER: ${emailConfigs.provider}`);
  }
}
